...
void IppFilterLaplacian(IppByteImage& imgSrc, IppByteImage& imgDst) {
	...
	for (j = 1; j < h - 1; j++) {
		for (i = 1; i < w - 1; i++) {
			sum = pSrc[j - 1][i] + pSrc[j][i - 1] 
				+ pSrc[j + 1][i] + pSrc[j][i + 1] 
				- 4 * pSrc[j][i];
...
void IppFilterUnsharpMask(IppByteImage& imgSrc, IppByteImage& imgDst) {
	...
	for (j = 1; j < h - 1; j++) {
		for (i = 1; i < w - 1; i++) {
			sum = 5 * pSrc[j][i] - pSrc[j - 1][i] - pSrc[j][i - 1]
				- pSrc[j + 1][i] - pSrc[j][i + 1];
...
// 강의록 21p 확인!! (Question)
void IppFilterUnsharpMask2(IppByteImage& imgSrc, IppByteImage& imgDst) {
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void IppFilterMinimum(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void IppFilterMaximum(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void IppFilterClosing(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	// Closing Filter : 최대 필터 수행 후, 최소 필터 적용
	IppByteImage temp1, temp2;
	imgDst = imgSrc;
	for (int i = 0; i < 1; i++) {
		IppFilterMaximum(imgDst, temp1);
		imgDst = temp1;
	}
	for (int i = 0; i < 2; i++) {
		IppFilterMinimum(imgDst, temp2);
		imgDst = temp2;
	}
}


void IppFilterOpening(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	// Opening Filter : 최소 필터 수행 후, 최대 필터 적용
	IppByteImage temp1, temp2;
	imgDst = imgSrc;
	for (int i = 0; i < 1; i++) {
		IppFilterMinimum(imgDst, temp1);
		imgDst = temp1;
	}
	for (int i = 0; i < 2; i++) {
		IppFilterMaximum(imgDst, temp2);
		imgDst = temp2;
	}
}
